<template>
  <div class="p-6 bg-gray-50">
    ...
    >
      <h2 class="text-xl font-medium text-gray-700 mb-6">Resumo de Feedback</h2>
      <a-row :gutter="24">
        <a-col :span="6">
          <div class="stat-card">
          ...
          <div class="stat-card">
            ...
            <div class="stat-info">
              ...
              <div class="stat-label">
                Média
                <span class="text-xs text-gray-400">({{ totalRatings }})</span>
              </div>
            </div>
          </div>
        </a-col>
    ...
    <!-- Lista de Ocorrências com design moderno -->
    <div v-else>
      <transition-group
        name="fade-list"
        tag="div"
      ...
      >
        <nuxt-img
          src="/empty.svg"
          alt="Não há reclamações registradas no momento"
          width="180"
          height="180"
          class="opacity-80"
        />
        <p class="text-gray-500 text-lg font-light">
          Não há reclamações registradas no momento
        </p>
        <a-button type="primary" class="mt-2">Atualizar</a-button>
      </div>
    </div>
  </div>
</template>
  ...
  try {
    ...
    stastic.value = {
      ...
      answered: stasticData.data.filter(
        (occurrence) => occurrence.status === "ANSWERED"
      ).length,
...
const averageRating = computed(() => {
  ...
  const sum = ratings.value.reduce((acc, curr) => acc + curr.score, 0);
...
const loadRatings = async () => {
  ...
  try {
    ...
    totalRatings.value = data.meta.totalItems;
  } catch (error) {
